      ...
      <div className="bg-white w-full max-w-5xl h-full md:h-auto md:max-h-[90vh] md:rounded-[3rem] shadow-2xl flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-10 bg-white">
          <form onSubmit={(e) => { e.preventDefault(); onSubmit(formData); }} className="space-y-10">
            ...
            {/* PHẦN 4: GHI CHÚ */}
            <div className="space-y-6">
               <h3 className="text-sm font-black text-slate-600 uppercase tracking-widest border-l-4 border-slate-600 pl-3">4. Ghi chú thêm</h3>
               <div>
                  <label className="block text-xs font-black text-slate-400 uppercase mb-2 ml-1">Nội dung ghi chú</label>
                  <textarea 
                    value={formData.notes}
                    onChange={(e) => handleInputChange('notes', e.target.value)}
                    rows={4}
                    className="w-full bg-slate-50 border border-slate-200 rounded-3xl px-6 py-5 outline-none focus:ring-4 focus:ring-slate-500/10 font-medium text-slate-700 placeholder:text-slate-300 transition-all resize-none"
                    placeholder="Nhập ghi chú hoặc lưu ý đặc biệt cho giao dịch này..."
                  />
               </div>
            </div>
