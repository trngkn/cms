            ...
            <div className="flex items-center gap-1 mx-1 md:mx-2">
              {Array.from({ length: totalPages }).map((_, i) => {
                ...
                if (p === 1 || p === totalPages || (p >= currentPage - 1 && p <= currentPage + 1)) {
                  return (
                    <button key={p} onClick={() => { setCurrentPage(p); window.scrollTo(0,0); }} className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold transition-all ${currentPage === p ? 'bg-blue-600 text-white shadow-md' : 'text-gray-400 hover:bg-100'}`}>
                      {p}
                    </button>
                  );
                }
